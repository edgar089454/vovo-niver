        ...
        <div className="max-w-3xl mx-auto text-center">
          <div className="bg-stone-900 p-10 md:p-14 rounded-2xl shadow-xl border border-stone-800">
            ...
            <p className="text-stone-300 text-lg leading-relaxed font-light mb-8">
              ...
              Cada riso, cada desafio superado e cada momento compartilhado moldaram
              o alicerce sólido desta família. Hoje não é apenas um aniversário,
              é a celebração de um legado vivo.
            </p>

            <div className="flex justify-center">
              {/* ❗ CORRIGIDO: a classe font-script NÃO existe */}
              <span className="font-serif text-gold-500 text-xl font-bold">
                Salmos 92:12
              </span>
            </div>
          </div>
...
// Navbar elegante com efeito de scroll
  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled
          ? 'bg-stone-950/90 backdrop-blur-md shadow-sm py-3 border-b border-stone-800'
          : 'bg-transparent py-5'
      }`}
    >
          ...
          {['Timeline', 'Galeria', 'Mensagens'].map((item) => (
            <a
              ...
              className={`hover:text-gold-400 transition-colors ${
                isScrolled ? 'text-stone-400' : 'text-stone-300'
              }`}
            >
              {item}
            </a>
